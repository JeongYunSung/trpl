...
선언적 매크로는 선언한 순서대로 인터프리터가 파싱하는 개념의 매크로이다.

`macro_rules!` 구문을 사용해 정의할 수 있다.

## 절차적 매크로

절차적 매크로는 TokenStream을 통해 매크로로 넘겨받은 Token값들을 기준으로 코드를 생성하는 매크로이다.

절차적 매크로는 아래와 같이 세 가지가 있다.

1. 구조체와 열거형에 사용되며 특정 구현부를 파생시킬 수 있다 #[derive] 매크로 ( 보통 Trait의 기본구현을 많이 사용 함 )
2. 모든 아이템에 사용가능한 커스텀 속성을 정의하는 속성형 매크로 ( #[route(GET, "/")]와 같이 구분자를 통해 토큰을 받음 )
3. 함수 호출처럼 보이지만 지정된 토큰을 인수로써 사용하는 함수형 매크로 ( 토큰을 함수 파라미터로 넘김 )